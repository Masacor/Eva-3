...
export class SQLiteService {
  ...
  async abrirBaseDeDatos(dbName: string, encrypted: boolean, mode: string, version: number, readonly: boolean) {
  ...
  async recuperarConexion(dbName: string, readonly: boolean) {
    return await this.sqliteConnection.retrieveConnection(dbName, readonly);
  }

  async cerrarConexion(database:string, readonly?: boolean) {
    return await this.sqliteConnection.closeConnection(database, readonly? readonly : false);
  }

  async crearBaseDeDatos(options: capSQLiteUpgradeOptions) {
    return await this.sqlitePlugin.addUpgradeStatement(options);
  }

  async guardarNombreBaseDeDatos(dbName: string) {
